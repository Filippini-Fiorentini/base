...
void write_solution_file(const Mesh_2d& mesh, const Eigen::Matrix<fdm::Real, Eigen::Dynamic, 1>& vect, const std::string& filename)
	...
	if(vect_size != vect.rows())
		{
		std::cerr << "The Eigen vector and the mesh must have the same dimension" << std::endl;
		return;
		}
	std::ofstream output;
...
Real read_value(const std::string& filename, const std::string& name, Real default_value)
	...
	if(value == default_value)
		std::cout << "Warning: " << name << " is equal to default_value = " << default_value << std::endl;
	return value;
	}
